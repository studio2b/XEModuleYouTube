...
//12AUG2021(1.0.1.) - "$token_type" parameter is added to XFYoutubeVideos method. Array keys are wrapped with double quotation marks(""). Undefined variable "$data" got a value as NULL.
class XFYoutubeVideos {
	var $api_key, $token, $token_type;
	var $apiUri = "https://www.googleapis.com/youtube/v3/videos";
	
	function XFYoutubeVideos($token=NULL, $api_key=NULL, $token_type=NULL) {
	...
	function rate($id, $rating, $onBehalfOfContentOwner=null) {
		if(!is_null($this->token) && !is_null($id) && !is_null($rating)) {
			...
			$curlClass = new XFCurl("POST", $this->apiUri."/rate".$getParam, $header, $data = NULL);
			$return = json_decode($curlClass->body, true);
	...
	function getRating($id, $onBehalfOfContentOwner=null) {
		if(!is_null($this->token) && !is_null($id)) {
			...
			$curlClass = new XFCurl("GET", $this->apiUri."/getRating".$getParam, $header, $data = NULL);
			$return = json_decode($curlClass->body, true);
